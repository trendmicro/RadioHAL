...
#define RH_NRF24_MAX_MESSAGE_LEN (RH_NRF24_MAX_PAYLOAD_LEN)
...
\brief Send and receive unaddressed, unreliable datagrams by nRF24L01 and
compatible transceivers.

Supported transceivers include:
- Nordic nRF24 based 2.4GHz radio modules, such as nRF24L01
http://www.nordicsemi.com/eng/Products/2.4GHz-RF/nRF24L01 and other compatible
transceivers. 
- nRF24L01p with PA and LNA modules that produce a higher power output similar
to this one:
...
unreliable datagrams of arbitrary length to 28 octets per packet. Use one of
the Manager classes to get addressing and acknowledgement reliability, routing,
meshes etc.
...
This library provides functions for sending and receiving messages of up to 28
octets on any frequency supported by the nRF24L01, at a selected data rate.
...
All messages sent and received by this class conform to this packet format, as
specified by the nRF24L01 product specification:

- 1 octets PREAMBLE
- 3 to 5 octets NETWORK ADDRESS
- 9 bits packet control field             TODO what's this?
- 0 to 32 octets PAYLOAD of user message
- 2 octets CRC (added by the radio)

# Connecting nRF24L01 to Arduino

The electrical connection between the nRF24L01 and the Arduino require 3.3V,
the 3 x SPI pins (SCK, SDI, SDO), a Chip Enable pin and a Slave Select pin. If
you are using the Sparkfun WRL-00691 module, it has a voltage regulator on
board and can be should with 5V VCC if possible. The examples below assume the
Sparkfun WRL-00691 module

Connect the nRF24L01 to most Arduino's like this (Caution, Arduino Mega has
different pins for SPI, see below). Use these same connections for Teensy 3.1
(use 3.3V not 5V Vcc).
...
For an Arduino Leonardo (the SPI pins do not come out on the Digital pins as
    for normal Arduino, but only appear on the ICSP header)
...
For an Arduino Due (the SPI pins do not come out on the Digital pins as for
    normal Arduino, but only appear on the SPI header). Use the same
connections for Yun with 5V or 3.3V.
...
See the example program and Makefile in examples/raspi. Requires bcm2835
  library to be previously installed.

    cd examples/raspi
    make
    sudo ./RasPiRH

You can override the default settings for the CSN and CE pins in the NRF24()
constructor if you wish to connect the slave select CSN to other than the
normal one for your Arduino (D10 for Diecimila, Uno etc and D53 for Mega)

Caution: on some Arduinos such as the Mega 2560, if you set the slave select
pin to be other than the usual SS pin (D53 on  Mega 2560), you may need to set
the usual SS pin to be an output to force the Arduino into SPI master mode.
...
and digitalOut() functions to achieve a high enough SPI bus frequency. If you
observe reliable behaviour with the default hardware SPI RHHardwareSPI, but
unreliable behaviour with Software SPI RHSoftwareSPI, it may be due to slow CPU
performance.
...
Frequency accuracy may be debatable. For nominal frequency of 2401.000 MHz (ie
    channel 1), my Yaesu VR-5000 receiver indicated the center frequency for my
test radios was 2401.121 MHz. Its not clear to me if the Yaesu is the source of
the error, but I tend to believe it, which would make the nRF24l01 frequency
out by 121kHz.

The measured power output for a nRF24L01p with PA and LNA set to 0dBm output is
about 18dBm.

# Radio operating strategy and defaults

The radio is enabled all the time, and switched between TX and RX modes
depending on whether there is any data to send. Sending data sets the radio to
TX mode. After data is sent, the radio automatically returns to Standby II
mode. Calling waitAvailable() or waitAvailableTimeout() starts the radio in RX
mode.
...
Memory usage of this class is minimal. The compiled client and server sketches
are about 6000 bytes on Arduino. The reliable client and server sketches
compile to about 8500 bytes on Arduino. RAM requirements are minimal.
*/
...
public:
  ...
  typedef enum {
    ...
    TransmitPower0dBm,       ///< On nRF24, 0 dBm
                       // Sigh, different power levels for the same bit patterns
                       // on RFM73: On RFM73P-S, there is a Tx power amp, so
                       // expect higher power levels, up to 20dBm. Alas there is
                       // no clear documentation on the power for different
                       // settings :-(
  ...
  /// its own chip enable and slave select pin. After constructing, you must
  /// call init() to initialise the interface and the radio module \param[in]
  /// chipEnablePin the Arduino pin to use to enable the chip for
  ...
  /// normal SS pin for your Arduino (D10 for Diecimila, Uno etc, D53 for Mega,
  /// D10 for Maple)
  ...
  /// the SPI frequency (perhaps where you have other SPI shields, low voltages
  /// etc), call SPI.setClockDivider() after init()). -Flush the receiver and
  /// transmitter buffers
  ...
  /// Writes a single byte to the NRF24, and at the same time reads the current
  /// STATUS register \param[in] reg Register number, one of RH_NRF24_REG_*
  ...
  /// RH_NRF24_REG_* \param[in] dest Array to write the register values to. Must
  ...
  /// RH_NRF24_REG_* \param[in] src Array of new register values to write. Must
  ...
  /// the NRF24 NRF24_REG_00_CONFIG register when in Idle mode. This allows you
  /// to change some chip configuration for compatibility with libraries other
  /// than this one. You should not normally need to call this. Defaults to
  /// NRF24_EN_CRC| RH_NRF24_CRCO, which is the standard configuration for this
  /// library (2 byte CRC enabled). \param[in] mode The chip configuration to be
  /// used whe in Idle mode. \return true on success
  bool setOpMode(uint8_t mode);

  /// Sets the Network address.
  /// Only nodes with the same network address can communicate with each other.
  /// You can set different network addresses in different sets of nodes to
  /// isolate them from each other. Internally, this sets the nRF24 TX_ADDR and
  /// RX_ADDR_P0 to be the given network address. The default network address is
  /// 0xE7E7E7E7E7 \param[in] address The new network address. Must match the
  /// network address of any receiving node(s). \param[in] len Number of bytes
  /// of address to set (3 to 5). \return true on success, false if len is not
  /// in the range 3-5 inclusive.
  ...
  /// compatible devces. The others are for the nRF24. \param [in] data_rate The
  /// data rate to use for all packets transmitted and received. One of
  ...
  /// \return true on success (which does not necessarily mean the receiver got
  /// the message, only that the message was successfully transmitted).
  ...
  /// \return true if a complete, valid message has been received and is able to
  /// be retrieved by recv()
  bool available();
  ...
  /// finite time to wake from sleep mode. \return true if sleep mode was
