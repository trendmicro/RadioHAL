...
\brief Send and receive unaddressed, unreliable datagrams by nRF51 and nRF52
compatible transceivers.

Supported transceivers include Nordic nRF51 based 2.4GHz radio modules, such as
nRF51822 and other compatible chips, such as used in RedBearLabs devices like:

  - https://redbear.cc/product/retired.html
...
unreliable datagrams of arbitrary length to 254 octets per packet.

The
[nRF51822](https://www.nordicsemi.com/eng/Products/Bluetooth-Smart-Bluetooth-low-energy/nRF51822)
and
[nRF52832](https://learn.sparkfun.com/tutorials/nrf52832-breakout-board-hookup-guide)
is a complete SoC (system on a chip) with ARM microprocessor and 2.4 GHz radio,
which supports a range of channels and transmission bit rates. Chip antenna is
on-board.

The nRF51 transceiver is configured to use Enhanced Shockburst with no
acknowledgement and no retransmits. `TXADDRESS` and `RXADDRESSES:RXADDR0`
(i.e., pipe 0) are the logical address used. The on-air network address is set
in BASE0 and PREFIX0. SHORTS is used to automatically transition the radio
between Ready, Start and Disable.  No interrupts are used.

# Packet Format

All messages sent and received by this class conform to this packet format. It
is NOT compatible with the one used by RH_NRF24 and the nRF24L01 product
specification, mainly because the nRF24 only supports 6 bits of message length.
 ...
 - 0 to 251 octets PAYLOAD (possibly encrypted)
...
The sample programs are designed to be built using Arduino 1.6.4 or later using
the procedures outlined in http://redbearlab.com/getting-started-nrf51822/ or
with Sparkfun nRF52832 breakout board, with Arduino 1.6.13 and Sparkfun nRF52
boards manager 0.2.3 using the procedures outlined in
https://learn.sparkfun.com/tutorials/nrf52832-breakout-board-hookup-guide

# Radio Performance

At DataRate2Mbps (2Mb/s), payload length vs airtime: 0 bytes takes about 70us,
128 bytes takes 520us, 254 bytes take 1020us.  You can extrapolate linearly to
slower data rates.
...
The compiled client and server sketches are about 42k bytes on Arduino.  The
reliable client and server sketches compile to about 43k bytes on Arduino.
Unfortunately the Arduino build environmnet does not drop unused clsses and
code, so the resulting programs include all the unused classes ad code. This
needs to be revisited.  RAM requirements are minimal.
