...
// Message length and CRC are automatically encoded and decoded by the MRF89XA
#ifndef RH_MRF89_MAX_MESSAGE_LEN
 #define RH_MRF89_MAX_MESSAGE_LEN (RH_MRF89_MAX_PAYLOAD_LEN)
#endif
...
\brief Send and receive unaddressed, unreliable datagrams by Microchip MRF89XA
and compatible transceivers. and modules.

The Microchip MRF89XA
http://ww1.microchip.com/downloads/en/DeviceDoc/70622C.pdf is a low cost 900MHz
bancd transceiver chip. It is commonly used on preassembled modules with
supporting circcuits and antennas, such as the MRF89XAM9A
http://www.microchip.com/wwwproducts/Devices.aspx?product=MRF89XAM9A This class
supports all such modules

This base class provides basic functions for sending and receiving unaddressed,
unreliable datagrams of arbitrary length to 59 octets per packet. Use one of
...
construction of translators and frequency changers, etc. Each instance requires
2 chip select pins, and interrupt pin the standard 3 SPI pins.

Naturally, for any 2 radios to communicate that must be configured to use the
same frequency and data rate, and with identical network addresses.
...
- 3 octets PREAMBLE
- 2 to 4 octets sync word
- 1 octet message length bits packet control field (if in variable packet mode)
- 2 octets CRC (automatically added by the radio)

The payload is whitened. No Manchester encoding is used.

# Connecting MRF89XA to Arduino

The electrical connection between the MRF89XA and the Arduino require 3.3V, the
3 x SPI pins (SCK, SDI, SDO), a 2 Chip Select pins (/CSCON and /CSDAT) and an
interrupt.

Caution: the MRF89XA is a 3.3V part and is not tolerant of 5V inputs.
Connecting MRF89XA directly to a 5V MCU such as most Arduinos will damage the
MRF89XA.

Connect the MRF89XA to most 3.3V Arduinos or Teensy 3.1 like this (use 3.3V not
5V).
