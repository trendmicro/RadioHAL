...
bool RH_NRF51::init() {
  ...
  // These shorts will make the radio transition from Ready to Start to Disable
  // automatically for both TX and RX, which makes for much shorter on-air times
  NRF_RADIO->SHORTS =
...
void RH_NRF51::setModeRx() {
  if (_mode != RHModeRx) {
    ...
    // Radio will transition automatically to Disable state when a message is
    // received
...
void RH_NRF51::setModeTx() {
  if (_mode != RHModeTx) {
    ...
    // transmitter for this type of radio, so if a message is received and an
    // ACK or reply is sent to soon, the original transmitter may not see the
    // reply. So we delay here to make sure the receiver is ready. Yes, I know
    // this is very ugly
    delay(1);
...
void RH_NRF51::setEncryptionKey(uint8_t *key) {
  ...
  if (key) {
    ...
    NRF_CCM->SHORTS =
        (CCM_SHORTS_ENDKSGEN_CRYPT_Enabled << CCM_SHORTS_ENDKSGEN_CRYPT_Pos);
...
bool RH_NRF51::available() {
  if (!_rxBufValid) {
    ...
    setModeIdle();
#if RH_NRF51_HAVE_ENCRYPTION
    // If encryption is enabled, the decrypted message is not available yet, and
    // there seems to be no way to be sure when its ready, but a delay of 2ms is
    // enough
    if (_encrypting)
      delay(2);
#endif
