...
// This is the maximum RadioHead user message length that can be supported by this module
#define RH_E32_MAX_MESSAGE_LEN (RH_E32_MAX_PAYLOAD_LEN)
...
\brief Driver to send and receive unaddressed, unreliable datagrams via a EBYTE
E32-TTL-1W and similar serial radio transceiver.

Works with [E32-TTL-1W](http://www.ebyte.com/en/product-view-news.aspx?id=108).

Note: it should also be possible to use the E32-TTL-1W with the RadioHAL
RH_Serial module, which will also you to send longer packets, but will require
you to use the EBYTE Wireless Module Setting program to configure the radio
first. In this arrangement the E32 would act as a transparent serial
connection. This has not been tested by us.
...
Manager classes may use this class to implement reliable, addressed datagrams
and streams, mesh routers, repeaters, translators etc.

Naturally, for any 2 radios to communicate that must be configured to use the
same frequency and modulation scheme.

This Driver provides an object-oriented interface for sending and receiving
data messages with EBYTE RFM95/96/97/98(W), Semtech SX1276/77/78/7E32-TTL-1W9
and compatible radio modules. These modules implement long range LORA
transcivers with a transparent serial interface. With 1W power output the
manufacturer claims up to 6km range.

This Driver provides functions for sending and receiving messages of up to 53
octets on any frequency supported by the radio, in a range of data rates and
power outputs. Frequency can be set with 1MHz precision to any frequency from
410 to 441MHz.
...
We tested with Arduino Uno. We used SoftwareSerial on pins 6 and 7) to connect
to the E32 module, so we could continue to use the only hardware serial port
for debugging.
...
Other connection schems are possible provided the approporiate constructors are
used for SoftwareSerial and RH_E32

# Memory

The RH_RF95 driver requires non-trivial amounts of memory. The sample programs
all compile to about 8kbytes each, which will fit in the flash proram memory of
most Arduinos. However, the RAM requirements are more critical. Therefore, you
should be vary sparing with RAM use in programs that use the RH_E32 driver.
...
This radio supports a range of different data rates and powers. The lowest
speeds are the most reliable, however you should note that at 1kbps and with an
13 octet payload, the transmission time for one packet approaches 5 seconds.
Therefore you should be cautios about trying to send too many or too long
messages per unit of time, lest you monopolise the airwaves. Be a good
neighbour and use the lowest power and fastest speed that you can.

Forward Error Correction (FEC) is always enabled in these radios by RH_E32.

# Range

When running with a power output of 1W and at the slowest speed of 1kbps, this
module has an impressive range. We tested with:
...
We were able to get reliable reception over 7km (6 km over ocean and 1 km
    through low rise residential area). You can expect less range with lower
power outputs and faster speeds. You can expect less range in highrise cities.
You can expect more range with directional antennas. You can expect more range
with shorter messages.

# Transmitter Power
Caution: the maximum power output of this radio (1W = 30dbM) is almost
certainly more than the permitted power level for unlicensed users in the ISM
bands in most countries. Be sure you comply with your local regulations. Be a
good neighbour and use the lowest power and fastest speed that you can.
