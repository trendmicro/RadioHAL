...
void RH_RF69::handleInterrupt() {
  ...
  if (debug()) {
    _printer->print("[RHAL] RH_RF69_REG_27_IRQFLAGS1 = ");
    _printer->println(irqflags1);

    _printer->print("[RHAL] RH_RF69_REG_28_IRQFLAGS2 = ");
  ...
  if (_mode == RHModeTx && (irqflags2 & RH_RF69_IRQFLAGS2_PACKETSENT)) {
    ...
    _txGood++;
    if (debug())
      _printer->println("[RHAL] PACKETSENT");
  }
  ...
  if (_mode == RHModeRx && (irqflags2 & RH_RF69_IRQFLAGS2_PAYLOADREADY)) {
    ...
    if (debug())
      _printer->println("[RHAL] PAYLOADREADY");
  }

  if (_mode == RHModeRx && (irqflags2 & RH_RF69_IRQFLAGS2_FIFOFULL)) {
    if (debug())
      _printer->println("[RHAL] FIFOFULL");
  }
}
...
void RH_RF69::readFifo() {
  ...
  // TODO decide what to do here - we should pre-read the length from the
  // register to save time if (_fixedPayloadLen > 0) // we're in fixed payload
  // len mode: trust the fixed length
  //    payloadlen = _fixedPayloadLen;
  // else
  payloadlen = _spi.transfer(
      0); // We're in variable payload len mode: first byte is payload len

  if (debug()) {
    _printer->print("[RHAL] Reading data off the RX FIFO: ");
...
void RH_RF69::setSyncWords(const uint8_t *syncWords, uint8_t len) {
  ...
  if (debug()) {
    _printer->print("[RHAL] 1 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }

  // not nullpointer, not 0, non zero length, within max sync word length
  if (syncWords && *syncWords && len && len <= 4) {
    if (debug())
      _printer->println(
          "[RHAL] Writing sync words into RH_RF69_REG_2F_SYNCVALUE1");
    spiBurstWrite(RH_RF69_REG_2F_SYNCVALUE1, syncWords, len);
    syncconfig |= RH_RF69_SYNCCONFIG_SYNCON; // enable syncon
    if (debug()) {
      _printer->print("[RHAL] 2 RH_RF69_REG_2E_SYNCCONFIG = ");
      _printer->println(syncconfig);
    }
  } else {
    if (debug()) {
      _printer->println("[RHAL] Deactivating sync words detection!!!!!");
      _printer->println(
          "[RHAL] If you really want to go promoscious mode, remember "
          "to set FifoFillCondition (register bit 0x2e:6) to 1 and "
          "disable CRC check (register bit 0x37:4 to 0)");
    }
    syncconfig &= ~RH_RF69_SYNCCONFIG_SYNCON; // disable syncon
    if (debug()) {
      _printer->print("[RHAL] 3 RH_RF69_REG_2E_SYNCCONFIG = ");
      _printer->println(syncconfig);
    }
  ...
  if (debug()) {
    _printer->print("[RHAL] 4 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
  syncconfig |= (len - 1) << 3;
  if (debug()) {
    _printer->print("[RHAL] 5 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
  spiWrite(RH_RF69_REG_2E_SYNCCONFIG, syncconfig);

  if (debug()) {
    _printer->print("[RHAL] 6 RH_RF69_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
}
...
bool RH_RF69::send(const uint8_t *data, uint8_t len) {
  ...
  if (debug()) {
    _printer->print("[RHAL] Preparing a packet holding ");
    _printer->print(len);
