...
RH_RF69::RH_RF69(uint8_t slaveSelectPin, uint8_t resetPin, uint8_t interruptPin,
                 RHGenericSPI &spi)
...
bool RH_RF69::checkPayloadLen(uint8_t len) {
  if (_variablePayloadLen)
    return (len + _sizeOfPayloadLen) <= RH_RF69_MAX_MESSAGE_LEN;

  return len <= RH_RF69_MAX_MESSAGE_LEN;
}

void RH_RF69::setPacketFormat(bool fixed, uint8_t len) {
  if (len > RH_RF69_MAX_MESSAGE_LEN || len == 0)
    _fixedPayloadLen = RH_RF69_MAX_MESSAGE_LEN;
  else
    _fixedPayloadLen = len;

  _variablePayloadLen = !fixed;

  uint8_t packetconfig1 = spiRead(RH_RF69_REG_37_PACKETCONFIG1);

  if (_variablePayloadLen)
    packetconfig1 |= (0b1 << 8); // set 8th bit
  else
    packetconfig1 &= ~(0b1 << 8); // unset 8th bit

  spiWrite(RH_RF69_REG_37_PACKETCONFIG1, packetconfig1);
...
bool RH_RF69::init() {
  ...
  //    spiWrite(RH_RF69_REG_2E_SYNCCONFIG, RH_RF69_SYNCCONFIG_SYNCON); // auto,
  //    tolerance 0
...
/**
 * @brief Low level function that reads the FIFO
 *
 * Note that, differently from the original RadioHead send method, this
 * extended one does not assumes any addressing headers. It just receives the
 * payload as is. Moreover, if the radio is in fixed payload length mode, the
 * first byte after preamble + sync words is already part of the actual
 * payload. Instead, if in variable payload length mode, that first byte is the
...
void RH_RF69::readFifo() {
  ...
  uint8_t payloadlen = _fixedPayloadLen;
...
void RH_RF69::setDeviation(float deviation) {
  // Fdev = FDEV / FSTEP
  uint32_t fdev = (uint32_t)((deviation) / RH_RF69_FSTEP);
...
void RH_RF69::setTxPower(int8_t power, bool ishighpowermodule) {
  ...
  if (ishighpowermodule) {
    ...
    if (_power <= 13) {
      ...
      palevel =
          RH_RF69_PALEVEL_PA1ON | ((_power + 18) & RH_RF69_PALEVEL_OUTPUTPOWER);
    } else if (_power >= 18) {
      // +18dBm to +20dBm
      // Need PA1+PA2
      // Also need PA boost settings change when tx is turned on and off, see
      // setModeTx()
    ...
    palevel =
        RH_RF69_PALEVEL_PA0ON | ((_power + 18) & RH_RF69_PALEVEL_OUTPUTPOWER);
  }
 ...
 */
void RH_RF69::setPromiscuous(bool promiscuous) {
  ...
  if (promiscuous) {
    syncconfig |= RH_RF69_SYNCCONFIG_SYNCON; // enable syncword detection
    syncconfig |=
        (0b1 << 6); // set FifoFillCondition (by setting register bit 0x2e:6)
    packetconfig1 &=
        ~(0b1 << 4); // disable CRC check (by un-setting register bit 0x37:4)
  } else {
    syncconfig &= ~RH_RF69_SYNCCONFIG_SYNCON; // disable syncword detection
    syncconfig &= ~(
        0b1 << 6); // unset FifoFillCondition (by unsetting register bit 0x2e:6)
    packetconfig1 |=
        (0b1 << 4); // disable CRC check (by setting register bit 0x37:4)
  }
...
#ifdef RHAL_DEBUG
  if (debug()) {
    _printer->print("1 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
#endif

  // good sync words: not nullpointer, not 0, non zero length, within max sync
  // word length
  if (syncWords && *syncWords && len && len <= 4) {

#ifdef RHAL_DEBUG
    if (debug())
      _printer->println("Writing sync words into RH_RF69_REG_2F_SYNCVALUE1");
#endif
...
#ifdef RHAL_DEBUG
    if (debug()) {
      _printer->print("2 RH_RF69_REG_2E_SYNCCONFIG = ");
      _printer->println(syncconfig);
    }
#endif
  } else { // null sync words: unsetting
#ifdef RHAL_DEBUG
    if (debug()) {
      _printer->println("Deactivating sync words detection!");
      _printer->println("If you really need full promoscious mode, use "
...
#ifdef RHAL_DEBUG
    if (debug()) {
      _printer->print("3 RH_RF69_REG_2E_SYNCCONFIG = ");
      _printer->println(syncconfig);
    }
#endif

    if (len ==
        0) // the filter goes up to len-1 (so if you want zero, set this to 1)
      len++;
  }
...
#ifdef RHAL_DEBUG
  if (debug()) {
    _printer->print("4 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
#endif
  syncconfig |= (len - 1) << 3;
#ifdef RHAL_DEBUG
  if (debug()) {
    _printer->print("5 RH_RF69_REG_2E_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
#endif
  spiWrite(RH_RF69_REG_2E_SYNCCONFIG, syncconfig);
...
#ifdef RHAL_DEBUG
  if (debug()) {
    _printer->print("6 RH_RF69_SYNCCONFIG = ");
    _printer->println(syncconfig);
  }
#endif
}
 ...
 * @brief Send payload.
 *
 * Write payload on the FIFO and put the radio in TX mode. Preamble and sync
 * words are prepended by the radio automatically. Note that, differently from
 * the original RadioHead send method, this extended one does not prepend any
 * addressing headers. It just sends the payload as is.
 *
 * @param data Pointers to byte array to be sent.
 * @param len Number of bytes to send.
 *
 * @return Whether the transmission was successful.
 */
bool RH_RF69::send(const uint8_t *data, uint8_t len) {
  if (len > RH_RF69_MAX_MESSAGE_LEN || len < 1)
    return false;
...
#ifdef RHAL_DEBUG
    if (debug())
      _printer->println("Writing length byte on the TX FIFO");
#endif
    _spi.transfer(len);
  }
