...
// filling and emptying of the FIFO.
#ifndef RH_RF69_MAX_MESSAGE_LEN
#define RH_RF69_MAX_MESSAGE_LEN RH_RF69_MAX_ENCRYPTABLE_PAYLOAD_LEN
#endif
...
// as a compiler definition to change the default value of the
// ishighpowermodule argument to setTxPower to true
//
// #define RFM69_HW
#ifdef RFM69_HW
...
///   Upeload the code with an FTDI adapter set to 3.3V.
...
/// filter)
/// - 1 octet RH_RF69 payload length (optional)
/// - 0 to 64 octets DATA
...
public:
  ...
  /// these symbolic definitions and not their integer equivalents: its possible
  /// that new values will be introduced in later versions (though we will try
  /// to avoid it). CAUTION: some of these configurations do not work corectly
  ...
  typedef enum {
    ...
    FSK_Rb55555Fd50, ///< FSK, Whitening, Rb = 55555kbs,Fd = 50kHz for RFM69 lib
                     ///< compatibility
  ...
  /// select the RF69 before accessing it. Defaults to the normal SS pin for
  /// your Arduino (D10 for Diecimila, Uno etc, D53 for Mega, D10 for Maple)
  /// \param[in] interruptPin The interrupt Pin number that is connected to the
  /// RF69 DIO0 interrupt line. Defaults to pin 2. Caution: You must specify an
  /// interrupt capable pin. On many Arduino boards, there are limitations as to
  /// which pins may be used as interrupts. On Leonardo pins 0, 1, 2 or 3. On
  /// Mega2560 pins 2, 3, 18, 19, 20, 21. On Due and Teensy, any digital pin. On
  /// Arduino Zero from arduino.cc, any digital pin other than 4. On Arduino M0
  /// Pro from arduino.org, any digital pin other than 2. On other Arduinos pins
  /// 2 or 3. See http://arduino.cc/en/Reference/attachInterrupt for more
  ...
  RH_RF69(uint8_t slaveSelectPin = SS, uint8_t resetPin = 4,
  ...
  /// The measurement is uncalibrated and without calibration, you can expect it
  /// to be far from correct. \return The measured temperature, in degrees C
  /// from -40 to 85 (uncalibrated)
  int8_t temperatureRead();

  /// Sets the transmitter and receiver
  /// centre frequency
  /// \param[in] centre Frequency in MHz. 240.0 to 960.0. Caution, RF69 comes in
  /// several different frequency ranges, and setting a frequency outside that
  /// range of your radio will probably not work \param[in] afcPullInRange Not
  /// used \return true if the selected frquency centre is within range
  ...
  /// This is a low level device access function, and should not normally ned to
  /// be used by user code. Instead can use stModeRx(), setModeTx(),
  ...
  /// the power level with the ishighpowermodule flag set to true. Else you wil
  /// get no measurable power output. Simlarly if you are not using a high power
  /// module, you must NOT set the ishighpowermodule (which is the default)
  /// \param[in] power Transmitter power level in dBm. For RF69W
  ...
  /// valid values are from -2 to +20. Caution: at +20dBm, duty cycle is limited
  /// to 1% and a maximum VSWR of 3:1 at the antenna port. \param
  ...
  /// including the data rate, bandwidths etc. You can use this to configure the
  /// modem with custom configurations if none of the canned configurations in
  ...
  /// starts the transmitter. Note that a message length of 0 is NOT permitted.
   ...
   * to set them to some custom values when not in promiscuous mode. Calling
   * this method will just disable sync-word-based filtering.
   *
  ...
  /// Sets the sync words for transmit and receive. if NULL, sync word
  /// filtering is disabled on the radio, which means that the radio will
  /// receive and accept packets with any sync words.
  /// \param[in] syncWords Array of sync words, 1 to 4 octets long. NULL if no
  /// sync words to be used. \param[in] len Number of sync words to set, 1 to 4.
  /// 0 if no sync words to be used.
  ...
  /// \param[in] key The key to use. Must be 16 bytes long. The same key must be
  /// installed in other instances of RF69, otherwise communications will not
  /// work correctly. If key is NULL, encryption is disabled, which is the
  /// default.
  ...
  /// transmitting or receiving), allowing you to control the idle mode power
  /// requirements at the expense of slower transitions to transmit and receive
  /// modes. By default, the idle mode is RH_RF69_OPMODE_MODE_STDBY, but eg
  /// setIdleMode(RH_RF69_OPMODE_MODE_SLEEP) will provide a much lower idle
  /// current but slower transitions. Call this function after init().
...
protected:
  ...
  /// RF69. Called automatically by isr*() Should not need to be called by user
  /// code.
...
protected:
  ...
  /// Time in millis since the last preamble was received (and the last time the
  /// RSSI was measured)
